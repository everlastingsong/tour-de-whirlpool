...
async function main() {
  ...
  //LANG:KR V0 트랜잭션용 ALT를 찾기 위해 Lookup Table Fetcher 사용 가능
  //LANG:KR devnet에서는 Orca가 제공하는 Lookup Table Fetcher가 없어 undefined로 설정
  //LANG:KR mainnet에서는 아래 코드로 Lookup Table Fetcher 생성
  //
  ...
  //LANG:KR DEVNET_WHIRLPOOLS_CONFIG에 속한 모든 풀을 조회
  ...
  const routingOptions: RoutingOptions = {
    ...
    //LANG:KR 라우트 분할 개수 및 각 라우트별 할당 비율 변경 범위 지정 가능
  };
  const selectionOptions: RouteSelectOptions = {
    ...
    //LANG:KR V0 트랜잭션 지원 여부 지정 (기본값 true)
    ...
    //LANG:KR 이미 생성된 ATA 제공 (undefined면 온체인에서 조회)
    //LANG:KR 동일 작업 반복 시 생성된 ATA 리스트 지정으로 성능 향상 가능
  ...
  try {
    ...
    //LANG:KR 라우트 정보 출력 (devSAMO/devUSDC, devTMAC/devUSDC를 거쳐 스왑하는 시나리오 예상)
